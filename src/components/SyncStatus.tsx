...
export default function SyncStatus({ lastSyncAt, onSync, isSyncing, syncResult, syncError, isFullSync }: SyncStatusProps) {
  ...
  // The nightly job syncs before sending expiry emails and removing tags, so a stale
  // timestamp means that job has stopped running and the membership data can't be trusted.
  const syncAgeDays = lastSyncAt
    ? (Date.now() - new Date(lastSyncAt).getTime()) / 86400000
    : Infinity;
  const isStale = syncAgeDays > 2;
    ...
    <div className="flex flex-col items-end gap-2">
      <div className="flex items-center gap-4">
        <div className="text-right">
          ...
          <div className={`text-sm ${isStale ? 'text-[#b45309] font-medium' : 'text-[#202223]'}`}>
            {lastSyncAt
      ...
      {isStale && !isSyncing && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 text-sm text-amber-800 flex items-center gap-2">
          ...
          <span>
            Membership data is out of date &mdash; the nightly sync hasn't run
            {lastSyncAt ? ` since ${formatDistanceToNow(new Date(lastSyncAt), { addSuffix: true })}` : ''}.
            Renewal dates and Quack tags may be wrong until you sync.
          </span>
        </div>
      )}
